...
CONTROL_QA = [
    ...
    (
        "Должен ли быть секретным ключ шифра при симметричном шифровании?",
        "Да, обязательно. В симметричном шифровании один и тот же ключ используется для "
        "шифрования и дешифрования, поэтому его раскрытие полностью компрометирует канал. "
        "Ключ передаётся участникам по защищённому каналу заранее.",
    ),
    (
        "Кто может знать алгоритм шифрования?",
        "Все — пользователи, разработчики, оценщики безопасности. Открытость алгоритма "
        "обеспечивает его независимую проверку и доверие к системе (принцип Керкгоффса).",
    ),
    (
        "Кто должен знать ключ шифра?",
        "Только законные участники защищённого канала — отправитель и получатель. "
        "Ключ является единственным секретом в симметричной системе. Его компрометация "
        "равносильна полной потере защиты.",
    ),
    (
        "Что делать, если размер ключа меньше размера текста?",
        "В шифре простой замены ключ фиксирован (перестановка алфавита) и не зависит от "
        "длины текста. В потоковых и XOR-шифрах короткий ключ циклически повторяется — "
        "это снижает стойкость. Идеальный вариант — одноразовый ключ длины текста (Вернам).",
    ),
    (
        "В чём заключается идея шифра простой замены?",
    ...
    (
        ...
        "Они совпадают по размеру (биекция между символами). В данном задании "
        "шифрообразования — двузначные числа, буквы открытого текста — символы "
        "русского алфавита; размеры алфавитов одинаковы.",
    ),
    (
        ...
        "ей код будет встречаться точно с той же частотой 9 %. Именно это свойство "
        "позволяет взламывать шифр простой замены частотным анализом.",
    ),
    (
        "Сколько уникальных вариантов ключа можно получить для заданного размера блока?",
        "Для 33-символьного алфавита ключ — это биективная функция σ: A → A, то есть "
        "перестановка. Количество перестановок равно 33! ≈ 8.68·10³⁶. Несмотря на "
        "огромное пространство ключей, частотный анализ взламывает шифр быстро при "
        "достаточной длине шифртекста.",
    ),
]
