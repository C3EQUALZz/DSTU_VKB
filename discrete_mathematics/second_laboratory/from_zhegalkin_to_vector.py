...
def parse_input(string_polynom: str) -> sympy.Xor:
    ...
    return sympy.Xor(
        *[monom for monom in map(str.strip, re.split(r"\^|\bxor\b", string_polynom))]
    )
...
def vector_from_polynom(polynomial_str: str):
    ...
    variable_names = sympy.symbols(
        " ".join(sorted(set(re.findall(r"\b[a-zA-Z]\b", polynomial_str))))
    )
    ...
    list_with_results = [
        int(bool(parsed_input.subs(dict(zip(variable_names, row)))))
        for row in itertools.product([0, 1], repeat=len(variable_names))
    ]
