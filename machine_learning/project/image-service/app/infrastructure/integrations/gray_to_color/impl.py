...
class KerasImageMessageColorizationModel(BaseImageGrayScaleToColorConverter):
    SIZE: Final[int] = 160
    ...
    def __resize_with_padding(self, img: np.ndarray) -> np.ndarray:
        ...
        resized: np.ndarray = cv2.resize(img, (new_w, new_h))
    ...
    def postprocess(self, prediction: np.ndarray) -> bytes:
        ...
        prediction: np.ndarray = cast("np.ndarray[bytes]", np.squeeze(prediction, axis=0))
        prediction: np.ndarray = cast("np.ndarray[bytes]", (np.clip(prediction, 0, 1) * 255).astype(np.uint8))
