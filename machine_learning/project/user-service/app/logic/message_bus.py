...
class MessageBus:
    ...
    async def handle(self, message: Union[AbstractEvent, AbstractCommand]) -> None:
        ...
        while not self._queue.empty():
            ...
            if isinstance(message, AbstractEvent):
                ...
                raise MessageBusMessageError("Message bus message should be eiter of Event type, or Command type")
    ...
    async def _handle_command(self, command: AbstractCommand) -> None:
        handler: AbstractCommandHandler[AbstractCommand] = self._command_handlers[
            type(command)
        ]
