...
from cryptography_methods.domain.common.errors.time_errors import (
    CreationTimeFormatError,
    CreationTimeFutureError,
)
from cryptography_methods.domain.common.values.base import BaseValueObject


@dataclass(frozen=True, eq=True, unsafe_hash=True)
class CreateTime(BaseValueObject):
    value: datetime

    @override
    def _validate(self) -> None:
        if self.value.tzinfo != UTC:
            raise CreationTimeFormatError(
                f"CreationTime must be timezone-aware with UTC, "
                f"not this: {self.value.tzinfo!s}",
            )
        if self.value > datetime.now(UTC):
            raise CreationTimeFutureError(
                "CreationTime cannot be in the future",
            )
