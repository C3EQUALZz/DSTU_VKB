...
class MenuPresenter(IMenuPresenter):
    def __init__(
        ...
        self._map_name_and_view: dict[str, type[ITaskView[T]]] = {}

    @override
    def attach_presenter(self, view: IMainMenuView) -> None:
        self._view: IMainMenuView = view

    def register_view(self, name: str, view: type[ITaskView[T]]) -> None:
        self._map_name_and_view[name] = view

    @override
    def on_menu_selection(self, algorithm_key: str) -> None:
        view: ITaskView[T] = self._map_name_and_view[algorithm_key](self.container) # type: ignore
