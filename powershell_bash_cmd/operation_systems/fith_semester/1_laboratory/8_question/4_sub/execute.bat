...
rem /f - строки и содержимое файлов
rem delims для построчной обработки
rem dir /b %dir2%\* выполняется изначально (\b в базовом формате)
echo Файлы, присутствующие в обоих каталогах:
for /f "delims=" %%f in ('dir /b "%dir2%\*"') do (
    rem Выводит и не учитывает регистр
