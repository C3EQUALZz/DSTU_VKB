...
РЕШЕНИЕ НЕ ПРОХОДИТ ПО СКОРОСТИ ИЗ-ЗА PYTHON. Решение на 100 баллов у C++
"""
...
class TravelingSalesmanGraph:
    def __init__(self, coordinates: Sequence[Tuple[float, float]]) -> None:
        ...
        self.distance_matrix: List[ArrayType[float]] = [
            array("d", [float("inf")] * self.num_vertices)
    ...
    def _initialize_distance_matrix(self) -> None:
        ...
        for mask, u in product(range(1 << self.num_vertices), range(self.num_vertices)):
            ...
            for v in filter(
                lambda vertex: not mask & (1 << vertex), range(self.num_vertices)
            ):  # Генератор для вершин
                next_mask: int = mask | (1 << v)
                minimal_distance_matrix: float = self.distance_matrix[next_mask][v]
                min_distance: float = min(
                    minimal_distance_matrix,
                    current_distance + self._calculate_distance(u, v),
                )
    ...
    def reconstruct_path(self, end_vertex: int, final_mask: int) -> Sequence[int]:
        ...
        while current_vertex != 0:
            ...
            current_vertex: int = next(
                prev_vertex
                for prev_vertex in range(self.num_vertices)
...
def main() -> None:
    ...
    coordinates = cast(
        List[Tuple[int, int]],
        [tuple(map(float, input().split())) for _ in range(num_vertices)],
    )
