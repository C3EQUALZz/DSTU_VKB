...
/// Инициализирует красивый pretty-логгер и color-eyre отчёты.
///
/// Использует переменную `RUST_LOG` для фильтра. По умолчанию — `INFO`.
/// Пример: `RUST_LOG=debug cargo run -p lab_01_rsa`.
///
/// # Errors
/// Возвращает ошибку, если повторно вызвать инициализацию или color-eyre уже установлен.
pub fn init() -> Result<()> {
