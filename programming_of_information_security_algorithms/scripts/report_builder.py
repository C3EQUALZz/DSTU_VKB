"""Генератор docx-отчётов по курсу «Программирование алгоритмов защиты информации».

Шаблон титульного листа повторяет эталон ДГТУ.
Преподаватель — Савельев В.А., студент — Ковалев Данил Петрович, ВКБ43.

Поддерживает три типа вставок:
- add_listing(code, caption) — моноширинный текст (для коротких кусков кода/вывода);
- add_image(path, caption)   — PNG-скриншот (freeze для кода и логов);
- add_math(omml_xml)         — OMML-формула.
"""
...
STUDENT_GROUP = "ВКБ43"
TEACHER_DEFAULT = "Савельев В.А."
DISCIPLINE = "Программирование алгоритмов защиты информации"
CITY = "Ростов-на-Дону"
YEAR = "2026"
...
class LabMeta:
    ...
    kind: str = "Лабораторная работа"  # "Лабораторная работа" / "Практическая работа"
...
def add_title_page(doc: _Document, meta: LabMeta) -> None:
    ...
    for _ in range(4):
        _add_blank_single(doc)
    _add_centered(doc, f"{meta.kind} №{meta.number}", bold=True, size=Pt(16))
...
def add_listing(doc: _Document, code: str, caption: str | None = None) -> None:
    """Моноширинный листинг (для коротких фрагментов кода/вывода)."""
...
def add_image_parts(
    ...
    *,
    ...
    """Вставляет {name}.png; если такой файл отсутствует, ищет {name}_1.png, {name}_2.png …
    и вставляет их с подписью «caption (часть K из N)»."""
    one = base_dir / f"{name}.png"
    if one.exists():
        add_image(doc, one, caption=caption, width_cm=width_cm)
        return
    parts = sorted(base_dir.glob(f"{name}_*.png"))
    if not parts:
        raise FileNotFoundError(f"Нет ни {name}.png, ни {name}_*.png в {base_dir}")
    total = len(parts)
    for idx, p in enumerate(parts, start=1):
        c = f"{caption} (часть {idx} из {total})" if total > 1 else caption
        add_image(doc, p, caption=c, width_cm=width_cm)
...
def add_image(
    ...
    *,
    ...
    """Вставка PNG-скриншота (терминал/код после freeze) с подписью по центру."""
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.paragraph_format.first_line_indent = Cm(0)
...
def add_table_simple(
    ...
    *,
    ...
    """Простая таблица. rows[0] — заголовок (если header=True)."""
    if not rows:
        return
