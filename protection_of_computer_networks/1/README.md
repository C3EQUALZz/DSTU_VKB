...
│   ├── report_builder.py        # обёртка над python-docx для отчёта
│   ├── build_report.py          # генератор отчёта
│   ├── manual_builder.py        # обёртка над python-docx для методички
│   └── build_manual.py          # генератор методички
├── artifacts/                   # txt-логи команд (создаются run_lab.sh)
├── Методичка.docx               # новая методичка (генерируется build_manual.py)
├── Методичка Болдырихин.docx    # исходная методичка (условие, не трогаем)
└── docs/
    ├── assets/dstu_logo.png
    ├── assets/manual/fig01..fig13   # скриншоты установки Docker
    └── reports/Ковалев Д.П. ВКБ43 1 лаба.docx
```

## Сборка методички

`Методичка.docx` генерируется скриптом — вручную её править не нужно, правки
вносятся в `scripts/build_manual.py` и документ пересобирается:

```sh
python3 scripts/build_manual.py
```

Листинги всех конфигураций (`docker/*`, `scripts/run_lab.sh`) читаются с диска
на этапе сборки, поэтому документ физически не может разойтись со стендом:
поправил конфиг — пересобрал методичку.

Что содержит документ: 13 рисунков (установка Docker в Windows + структура
каталога), 80 листингов, 26 таблиц. Структура — теория (раздел 1), архитектура
стенда (2), установка Docker (3), создание файлов стенда с разбором каждого
параметра (4), запуск (5), 11 шагов выполнения (6), диагностика и типовые
ошибки (7), справочник команд (8), требования к отчёту (9), критерии
оценивания (10), контрольные вопросы и практические задания без ответов,
приложения с полными исходниками.

Скриншоты берутся из `docs/assets/manual/`. Если их нет, генератор просто
вставит подпись без изображения — сборка не упадёт.
