...
Если `--zero/--one` не заданы, значения зависят от контейнера:

- для `--cover-text` берутся незаметные пары по умолчанию
  (например, для `size` — `28` и `29`, т.е. 14pt и 14.5pt);
- для `--cover-file` действует режим `auto`: нулевые биты оставляют
  исходное форматирование нетронутым, единичные получают минимальную
  дельту от значения самого run'а (+0,5 pt размера, +1 оттенок цвета,
  −1 % масштаба, +1 twip интервала). Все остальные части docx (стили,
  тема, rels) переносятся без изменений, поэтому контейнер визуально
  неотличим от оригинала. Decode распознаёт такой файл сам: детектор
  раскладывает значения на пары «база / база+дельта».
