...
class FileCompressor:
    ...
    def compress(self, source, filename, level=FastLzLevel.LEVEL1):
        ...
        destination += FastLZConstants.SIXPACK_MAGIC_IDENTIFIER
        ...
        while source_position < source_length:
            ...
            if compression_method == 1:
                ...
                # Update the checksum.
                checksum = update_adler32(1, result)
                ...
                checksum = update_adler32(1, buffer)
...
class FileDecompressor:
    ...
    def decompress(self, source):
        ...
        source_position = len(FastLZConstants.SIXPACK_MAGIC_IDENTIFIER)
        source_length = len(source)
        ...
        while source_position < source_length:
            ...
            if header.chunk_id == 1 and 10 < header.chunk_size < FastLZConstants.BLOCK_SIZE_DECOMPRESSION:
                ...
                decompressed_size = read_unsigned_integer_32_bit(buffer, 0)
            ...
            if header.chunk_id == 17 and decompressed_size:
                ...
                if header.chunk_options == 0:
                    ...
                    # Update the checksum.
                    checksum = update_adler32(1, compressed_buffer)
    ...
    def _read_chunk_header(buffer, start_position):
        ...
        header.chunk_id = read_unsigned_integer_16_bit(buffer, start_position)

        # Get the chunk options, taking the next 2 bytes.
        header.chunk_options = read_unsigned_integer_16_bit(buffer, start_position + 2)

        # Get the size, taking the next 4 bytes.
        header.chunk_size = read_unsigned_integer_32_bit(buffer, start_position + 4)

        # Get the checksum, taking the next 4 bytes.
        header.chunk_checksum = read_unsigned_integer_32_bit(buffer, start_position + 8)

        # Get the extra data, taking the next 4 bytes.
        header.chunk_extra = read_unsigned_integer_32_bit(buffer, start_position + 12)
