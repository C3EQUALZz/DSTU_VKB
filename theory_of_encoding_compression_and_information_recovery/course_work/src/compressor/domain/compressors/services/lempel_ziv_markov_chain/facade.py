...
class LZMACompressor(Compressor):
    ...
    def compress(self, file: File) -> CompressedFile:
        ...
        dest_path: FileName = FileName(
            str(converted_source_path_to_path.with_suffix(converted_source_path_to_path.suffix + ".xz"))
        )
        data: BytesIO = BytesIO()

        with lzma.open(data, "wb", preset=self._lzma_configuration.COMPRESS_LEVEL, format=lzma.FORMAT_XZ) as f_out:
            while chunk := file.data.read(self._lzma_configuration.CHUNK_SIZE):
                f_out.write(chunk)
        ...
        return self._file_service.create_compressed_file(
            file_name=dest_path, compression_type=CompressionType.LZMA, data=data
        )
    ...
    def decompress(self, file: CompressedFile) -> File:
        ...
        if not converted_path.suffix.endswith(CompressionType.LZMA):
            msg = f"Expected .xz file, not {converted_path.suffix}"
            raise CantDecompressThisFileError(msg)

        dest_path: Path = converted_path.with_suffix("")
        data: BytesIO = BytesIO()

        with lzma.open(file.data, "rb") as f_in:
