...
class CodeTable:
    ...
    def __post_init__(self) -> None:
        ...
        logger.info(
            f"Таблица информационных и кодовых слов {self.__class__.__name__} %s", data
        )

    def __iter__(self):
        return zip(
            self.information_words_column,
            self.code_words_column,
            self.hamming_weights_column,
        )
...
def create_code_table(generator_systematic_matrix: "GSystematicMatrix") -> CodeTable:
    ...
    info_words = np.array(
        list(itertools.product([0, 1], repeat=generator_systematic_matrix.k))
    )
    information_words_column = cast(np.ndarray[np.ndarray[int]], info_words)

    # Генерация кодовых слов
    code_words_column = np.mod(
        information_words_column @ generator_systematic_matrix.matrix, 2
    )
    ...
    return CodeTable(
        information_words_column, code_words_column, hamming_weights_column
    )
...
def find_errors(table: CodeTable) -> Errors:
    ...
    """
    d_min = np.min(
        table.hamming_weights_column[table.hamming_weights_column > 0]
    ).item()
