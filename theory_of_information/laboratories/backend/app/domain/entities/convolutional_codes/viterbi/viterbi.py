...
from app.domain.entities.convolutional_codes.viterbi.helpers import (
    bits_count, conv_parity, hamming_distance)
...
class Transitions(BaseEntity):
    ...
    def __post_init__(self) -> None:
        ...
        self.states = [
            [
                Edge(
                    new_state=((i_state << 1) & (2**self.n_state_bits - 1)) | bit,
    ...
    def __str__(self):
        ...
        for i, state in enumerate(self.states):
            ...
            lines = [
                " --{{}}/{{:0{}b}}--> {{:0{}b}}\n".format(
                    self.parity_len, self.n_state_bits
                ).format(b, state[b].parity, state[b].new_state)
                for b in (0, 1)
            ]
    ...
    def extract_parity_sequence(self, parity_sequence_bindata):
        ...
        while parity_selector:
            ...
            yield (
                parity_sequence_bindata.num & (parity_mask << parity_selector)
            ) >> parity_selector
    ...
    def decode(self, parity_sequence_bindata):
        ...
        # init trellis
        olds = [
            Node(INF, BinData(0, 0)) for i in range(self.n_states)
        ]  # aktualna metrika, data bits
        news = [
            Node(None, None) for i in range(self.n_states)
        ...
        for parity in gen:
            ...
            for i in range(self.n_states):
                for bit in (0, 1):
                    ...
                    if (
                        new_PM < news[t].metric
                    ):  # if this new candidate is better than existing new candidate
