import logging
from typing import AnyStr, List, Literal, cast

import numpy as np
from app.domain.entities.block_codes.code_table import (create_code_table,
                                                        find_errors)
...
class BlockCodesService:
    def encode(
        ...
        for block in blocks:
            ...
            index = np.where(
                (code_table.information_words_column == block).all(axis=1)
            )[0][0]
        ...
        return cast(
            np.ndarray[np.ndarray[str]], np.array(encoded_word, dtype=StringDType())
        )
    ...
    def decode(
        ...
        verification_systematic_matrix_transposed = (
            self.__get_verification_systematic_transposed_matrix(matrix, type_matrix)
        )
        ...
        for block in blocks:
            syndrome = (
                np.dot(block, verification_systematic_matrix_transposed.matrix) % 2
            )
        ...
        result = cast(
            np.ndarray[list[str, str, str]],
            np.array(
                [byte_chunks[i : i + 3] for i in range(0, len(byte_chunks), 3)],
                dtype=StringDType(),
            ),
        )
    ...
    def get_info_for_encoding(matrix: list[list[int]], type_matrix: Literal["G", "H"]):
        ...
        inverse_systematic_matrix = (
            gen_or_check_systematic_matrix.find_another_type_matrix()
        )
        ...
        if inverse_systematic_matrix.__class__.__name__ == "GSystematicMatrix":
            ...
            raise ValueError(
                f"Неправильный тип матрицы {inverse_systematic_matrix.__class__.__name__}"
            )
    ...
    def __get_verification_systematic_transposed_matrix(
        matrix: List[List[int]], type_matrix: Literal["G", "H"]
    ) -> "HSystematicMatrix":
        """
        ...
        logger.info(
            "Транспонированная проверочная систематическая матрица HSystematicMatrix %s",
