...
class EncodeCascadeCodeUseCase(BaseUseCase[EncodeCascadeCodeCommand]):
    ...
    async def __call__(self, command: EncodeCascadeCodeCommand) -> str:
        ...
        logger.info("Закодированная последовательность после блочного кодирования: %s", block_encoded)
        ...
        logger.info("Закодированная последовательность после алгоритма Витерби: %s", encoded_data)
        ...
        if command.add_errors:
            ...
            for i in range(0, len(encoded_data), bits_per_pixel):
                ...
                logger.info("Новый набор битов, где инвертировали на позиции 3: %s", pixel_bits)
...
class DecodeCascadeCodeUseCase(BaseUseCase[DecodeCascadeCodeCommand]):
    ...
    async def __call__(self, command: DecodeCascadeCodeCommand) -> np.ndarray[list[str, str, str]]:
        count_of_register: int = max(max(sublist) for sublist in command.indexes) + 1
        ...
        split_indices = np.linspace(0, len(binary_word), count_of_columns + 1, dtype=int)
