...
/**
 * Handler for revoking admin rights from a user by ID.
 * <p>
...
public final class RevokeAdminByIDCommandHandler {
    ...
    /**
     * Executes the revoke admin operation.
     *
     * @param data the command containing the user ID to revoke admin rights from
     * @return Mono that completes when admin rights are revoked
     * @throws UserNotFoundByIDError if the user is not found
     */
    public Mono<Void> run(RevokeAdminByIDCommand data) {
        log.info("Revoke admin: started. User id: '{}'.", data.user_id());
                ...
                .flatMap(currentUser -> {
                    ...
                    return userCommandGateway.readById(userId)
                            .flatMap(optionalUser -> {
                                ...
                                User userForRevokeAdmin = optionalUser.get();

                                // Toggle user admin role to false
                                accessService.toggleUserAdminRole(userForRevokeAdmin, false);
